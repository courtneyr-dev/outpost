...
**PHPUnit is pinned to `^9.6` deliberately** (`composer.json` require-dev, confirmed in CLAUDE.md Session A1 decision #8) — `10up/wp_mock` 1.x has a Mockery compatibility window that caps out before PHPUnit 10. Do not bump PHPUnit past 9.x without first resolving WP_Mock's compat; a well-meaning dependency update is the most common way this regresses.

**Test invariants to preserve** (see CLAUDE.md "Testing" section and Session A2 decision #8):
...
- No OR-assertions in defense-in-depth tests; auth-gate tests assert absence of side effects (no transients, no fired hooks), not just the returned status code. See CLAUDE.md "Testing".
...
- Commits use Emoji-Log going forward (see CLAUDE.md "Commit convention"): exactly one of `📦 NEW:` `👌 IMPROVE:` `🐛 FIX:` `📖 DOC:` `🚀 RELEASE:` `🤖 TEST:` `‼️ BREAKING:`, imperative mood.
